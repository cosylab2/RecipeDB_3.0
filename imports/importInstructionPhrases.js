...
(async () => {
  ...
  if (!file) throw new Error("Please provide path to Instruction_Phrases.csv");
  ...
  stream.on("data", (row) => {
    ...
    const _id = hashRow({ ...row, Recipe_ID: rid });

    ops.push(
      RecipeInstruction.updateOne(
        { _id },
        { $set: { ...row, Recipe_ID: rid, _id } },
        { upsert: true }
      )
    );
  });
  ...
  stream.on("end", async () => {
    console.log(`Queued ${ops.length} instruction-phrase upserts. Waiting for Mongo...`);
    await Promise.all(ops);
    console.log("✅ Instruction_Phrases import complete.");
